...
!!! info
    Scene Notes are designed primarily for **editor use during development and playtesting**.

!!! warning
    Notes will not appear in builds unless the runtime viewer is enabled.

!!! tip
    Use Play Mode notes during QA sessions to quickly report issues without leaving the game.

??? example "Installation Steps"

    1. Import **Scene Notes** into your project.
    2. Open **Window → Scene Notes Manager**.
    3. Enable **Play Mode Notes**.

```csharp
SceneNotesManager.Instance.CreateNote(
    "Player spawn point broken",
    transform.position
);
